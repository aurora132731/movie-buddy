...
function computeMatchStats(room) {
  ...
  return room.movies
    .map((movieId) => {
      const matched = participants.filter((participant) =>
        isPositiveVote(participant.swipes[movieId])
      );
      return {
        movieId,
        count: matched.length,
        total,
        names: matched.map((participant) => participant.name)
      };
    })
    .filter((entry) => entry.count >= 2)
    .sort((a, b) => b.count - a.count || a.movieId.localeCompare(b.movieId));
}

function computeMatches(room) {
  return computeMatchStats(room).map((entry) => entry.movieId);
}

function unanimousMatches(room) {
  const participants = Object.values(room.participants);
  if (participants.length < 2) return [];

  return room.movies.filter((movieId) =>
    participants.every((participant) => isPositiveVote(participant.swipes[movieId]))
  );
}

function roomPayload(room) {
  const matchStats = computeMatchStats(room);
  return {
    code: room.code,
    hostId: room.hostId,
    ...
    matches: matchStats.map((entry) => entry.movieId),
    matchStats,
    unanimousMatches: unanimousMatches(room),
...
async function applyRoomAction(room, body) {
  if (body.action === "start") {
    if (body.participantId !== room.hostId) {
  ...
  if (body.action === "swipe") {
    ...
    await writeRoom(room);
    const matchIds = unanimousMatches(room);
    return {
      room,
